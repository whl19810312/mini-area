...
// LiveKit removed - using Mediasoup
import { useRealtimeCharacterSync } from '../hooks/useRealtimeCharacterSync';
import MediasoupVideoPanel from './MediasoupVideoPanel';
...
import AreaDebugOverlay from './AreaDebugOverlay';
...
const MetaverseScene = forwardRef(({ currentMap, mapImage: mapImageProp, characters, currentCharacter, isEditMode = false, onReturnToLobby }, ref) => {
  ...
  // 디버그: user 객체 확인 (변경시에만)
  const previousUserMapRef = useRef({ userId: null, mapId: null });
  const userMapKey = `${user?.id}-${currentMap?.id}`;
  
  if (previousUserMapRef.current.key !== userMapKey) {
    console.log('🎬 MetaverseScene - user/map 변경:', { user: user?.username, currentMap: currentMap?.name });
    previousUserMapRef.current = { key: userMapKey, userId: user?.id, mapId: currentMap?.id };
  }
  ...
  const [isMediasoupPanelVisible, setIsMediasoupPanelVisible] = useState(true); // Mediasoup 화상통화 패널 - 항상 활성
  ...
  // handleWheel은 useEffect 내부로 이동 (non-passive listener)
    ...
    const wheelHandler = (e) => {
      ...
      setZoomScale(prevScale => {
        ...
        setPanOffset(prevOffset => {
          ...
          const newPanX = mouseX - (mouseX - prevOffset.x) * scaleChange;
          const newPanY = mouseY - (mouseY - prevOffset.y) * scaleChange;
  ...
  // Mediasoup 화상통화 패널 자동 표시 - 최초 1번만
  useEffect(() => {
    if (!currentMap) return;
    
    console.log('🎥 Mediasoup 화상통화 자동 시작');
    setIsMediasoupPanelVisible(true);
  }, [currentMap?.id]); // currentMap이 변경될 때만 실행
  ...
  useEffect(() => {
    ...
    */
    ...
    // 맵 입장 시 현재 캐릭터 위치 정보도 함께 전송 (중복 입장 방지)
    ...
    // Mediasoup를 사용하므로 WebRTC 자동 시작 제거
    // 사용자가 카메라 버튼을 클릭할 때 MediasoupVideoPanel이 열림
  ...
  }, [socket, currentMap?.id, user?.id]); // 필수 의존성만 포함
  ...
  */
  ...
  // Mediasoup video tile will be handled by MediasoupVideoPanel component
  ...
  return (
    <div className="metaverse-container">
      <NavigationBar 
        ...
        onToggleGroupCall={() => {
          // Mediasoup 화상통화 토글
          setIsMediasoupPanelVisible(!isMediasoupPanelVisible);
        }}
        groupCallActive={isMediasoupPanelVisible}
      ...
      {/* Mediasoup 화상통화 - 하단 중앙 고정, 맵 이동과 무관 */}
      {useMemo(() => {
        if (currentView !== 'metaverse' || !isMediasoupPanelVisible || !currentMap || !user?.id || !user?.username) {
          return null;
        }
        
        return (
          <MediasoupVideoPanel
            roomId={`map-${currentMap.id}`}
            userId={user.id}
            username={user.username}
            areaId='public' // 영역에 상관없이 고정된 영역 사용
            onClose={() => {
              console.log('🎥 MediasoupVideoPanel 닫기');
              setIsMediasoupPanelVisible(false);
            }}
          />
        );
      }, [currentView, isMediasoupPanelVisible, currentMap?.id, user?.id, user?.username])}
      
      {/* 현재 영역 표시 - 오브젝트 위치 (하단 중앙) */}
      {currentView === 'metaverse' && (
        <div style={{
          position: 'fixed',
          bottom: '20px',
          ...
          <span style={{ fontSize: '16px' }}>
            {charSync.currentAreaInfo?.type === 'public' ? '🌍' : '🔒'}
          </span>
          <span>
            {charSync.currentAreaInfo?.name || '알 수 없음'}
          </span>
        </div>
      )}
      ...
      {currentView === 'metaverse' && (
        <AreaDebugOverlay
          ...
          currentPosition={charSync.myPosition}
          currentArea={charSync.currentAreaInfo}
          mapScale={1}
        />
      )}
